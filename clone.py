...
def isImage(ImagePath):
    if os.path.isfile(ImagePath):
        ...
        raise ValueError('Path provided is not a valid file: {}'.format(ImagePath))
...
def loadImageAndMeasurement(dataPath, imagePath, measurement, images, measurements):
    ...
    """
    imagePath = 'IMG/'+imagePath.split('/')[-1]
    originalImage = isImage(dataPath + '/' + imagePath.strip())
...
X_train, y_train = loadImagesAndMeasurements(Path_To_Data, skipHeader=True)
model = leNetModel()
# model = nVidiaModel()
print('Training model')
trainAndSave(model, X_train, y_train, 'models/leNet_data.h5')
